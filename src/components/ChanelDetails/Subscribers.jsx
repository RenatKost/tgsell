...
const Subscribers = ({ stats = [], current }) => {
	...
	return (
		<div className='bg-white dark:bg-card rounded-xl border border-gray-100 dark:border-card-border shadow-sm px-4 pt-4 w-full'>
				...
				<div className='flex items-center gap-0.5 flex-shrink-0'>
					{TF.map((t, i) => (
						<button key={i} onClick={() => setTf(i)}
							...
							tf === i ? 'bg-[#27ACD2] text-white' : 'bg-gray-50 dark:bg-card-inner text-gray-400 hover:bg-gray-100 dark:hover:bg-card-hover'
							}`}>{t.label}</button>
					))}
			...
			<ChartModal open={modal} onClose={() => setModal(false)} stats={stats} title='Підписники' color='#27ACD2' dataKey='subscribers' seriesName='Підписники' />
		</div>
	);
};
